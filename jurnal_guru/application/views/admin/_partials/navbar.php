<div class="sidebar-header">
    <a href="index.html"><img class="main-logo" src="<?= base_url('assets/img/logo/logo.png') ?>" alt="" /></a>
    <strong><img src="<?= base_url('assets/img/logo/logosn.png') ?>" alt="" /></strong>
</div>
<div class="nalika-profile">
    <div class="profile-dtl">
        <a href="#"><img src="<?= base_url('assets/img/notification/4.jpg') ?>" alt="" /></a>
        <h2>Lakian <span class="min-dtn">Das</span></h2>
    </div>
</div>
<div class="left-custom-menu-adp-wrap comment-scrollbar">
    <nav class="sidebar-nav left-sidebar-menu-pro">
        <ul class="metismenu">
            <li class="active">
                <a href="#">
                        <i class="fa fa-home icon-wrap"></i>
                        <span class="mini-click-non">Dashboard</span>
                </a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-book icon-wrap"></i> <span class="mini-click-non">Data Mengajar</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-exchange icon-wrap"></i> <span class="mini-click-non">Permintaan Mengajar</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-money icon-wrap"></i> <span class="mini-click-non">Penarikan Saldo</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-envelope icon-wrap"></i> <span class="mini-click-non">Izin Guru</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-line-chart icon-wrap"></i> <span class="mini-click-non">Statistik Guru</span></a>
            </li>
            <center><h4>Data Master</h4></center>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-wpforms icon-wrap"></i> <span class="mini-click-non">Tahun Ajaran</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-wpforms icon-wrap"></i> <span class="mini-click-non">Data Jurusan</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-wpforms icon-wrap"></i> <span class="mini-click-non">Data Kelas</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-users icon-wrap"></i> <span class="mini-click-non">Data Guru</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-wpforms icon-wrap"></i> <span class="mini-click-non">Mata Pelajaran</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-calendar icon-wrap"></i> <span class="mini-click-non">Jadwal</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-gear icon-wrap"></i> <span class="mini-click-non">Pengaturan Aplikasi</span></a>
            </li>
            <center><h4>Data User</h4></center>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-user icon-wrap"></i> <span class="mini-click-non">Admin</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-user icon-wrap"></i> <span class="mini-click-non">Guru</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-user icon-wrap"></i> <span class="mini-click-non">Siswa</span></a>
            </li>
            <li>
                <a href="mailbox.html" aria-expanded="false"><i class="fa fa-user icon-wrap"></i> <span class="mini-click-non"></span></a>
            </li>
            <li>
                <a class="has-arrow" href="#" aria-expanded="false"><i class="fa fa-user icon-wrap"></i> <span class="mini-click-non"></span></a>
                <ul class="submenu-angle" aria-expanded="false">
                    <li><a title="Login" href="login.html"><span class="mini-sub-pro"></span></a></li>
                    <li><a title="Register" href="register.html"><span class="mini-sub-pro"></span></a></li>
                    <li><a title="Lock" href="lock.html"><span class="mini-sub-pro"></span></a></li>
                </ul>
            </li>
        </ul>
    </nav>
</div>